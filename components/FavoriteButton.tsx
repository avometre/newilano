...
export function FavoriteButton({ productSlug, className, activeClassName }: FavoriteButtonProps) {
  ...
  return (
    <div className={styles.container}>
      ...
      >
        <HeartIcon width={18} height={18} aria-hidden="true" fill={isFavorite ? '#ffffff' : 'none'} />
        {label}
      </button>
      {error ? (
        <span className={`${styles.message} ${styles.error}`} role="alert">
          {error}
        </span>
      ) : null}
    </div>
